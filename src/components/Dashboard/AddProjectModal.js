...
import { FaTimes, FaPlus, FaCalendarAlt } from 'react-icons/fa';
...
const AddProjectModal = ({ isOpen, onClose, onProjectAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.client || !formData.deadline) {
      alert(t('projects.requiredFields', 'Please fill in all required fields'));
      return;
    }
    ...
    try {
      ...
      const projectData = {
        ...
        clientMood: 'neutral', // Default mood
      ...
      if (onProjectAdded) {
        onProjectAdded({ id: docRef.id, ...projectData });
      }
      
      onClose();
    } catch (error) {
      console.error('Error adding project:', error);
      alert(t('projects.errorAdding', 'Error adding project. Please try again.'));
    } finally {
      setIsLoading(false);
    }
  };
